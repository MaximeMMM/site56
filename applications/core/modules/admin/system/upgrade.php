...
class _upgrade extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		\IPS\Output::i()->redirect( \IPS\Http\Url::external( "https://prowebber.ru/ips/ips-releases/" ) );
	}
	...
	public function _selectVersion( $data )
	{
		...
		else
		{
			...
			try
			{
				...
				if( \IPS\CIC AND \IPS\Cicloud\isManaged() AND isset( $response['blockms'] ) AND $response['blockms'] )
				{
		...
		{			
			...
			if( !\IPS\IPS_ALPHA_BUILD )
			{
				...
				if( (int) \IPS\Application::getAvailableVersion('core') > \IPS\Application::load('core')->long_version )
				{
